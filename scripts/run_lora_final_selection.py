...
OUTPUT_ROOT = ROOT / "outputs" / "analysis" / "lora_final_model_selection"
...
CANDIDATES = [
    Candidate(
        ...
        rationale="Original final adapter family and baseline for model selection.",
    ),
    Candidate(
        name="r8_both_dropout005_lr2e4",
        group="learning_rate",
        lora_r=8,
        lora_alpha=16,
        lora_dropout=0.05,
        learning_rate=2e-4,
        target_modules=BOTH_MODULES,
        rationale="Best single-seed validation macro-F1 in the exploratory tuning grid.",
    ),
    Candidate(
        name="r8_both_dropout010_lr1e4",
        group="dropout",
        lora_r=8,
        lora_alpha=16,
        lora_dropout=0.10,
        learning_rate=1e-4,
        target_modules=BOTH_MODULES,
        rationale="Moderate regularization candidate with strong validation performance.",
    ),
    Candidate(
        ...
        rationale="Best single-seed test macro-F1 in the exploratory grid; included for validation-based confirmation.",
    ),
]
...
RUN_NAME_ALIASES = {
    ...
    ("r8_both_dropout005_lr2e4", 42): "neutral_aware_lora_r8_both_dropout005_lr2e4_seed42",
    ("r8_both_dropout010_lr1e4", 42): "neutral_aware_lora_r8_both_dropout010_lr1e4_seed42",
...
def summarize_candidate(rows: list[dict]) -> list[dict]:
    summary = []
    names = sorted({row["candidate_name"] for row in rows})
    for name in names:
        group = [row for row in rows if row["candidate_name"] == name]
    ...
    return sorted(
        summary,
        key=lambda item: (
            item.get("val_macro_f1_mean", -1),
            -item.get("val_macro_f1_std", 999),
            item.get("test_macro_f1_mean", -1),
        ),
        reverse=True,
    )
...
def write_summary() -> None:
    ...
    candidate_rows = summarize_candidate(rows)
    write_csv(OUTPUT_ROOT / "runs.csv", rows)
    write_csv(OUTPUT_ROOT / "candidates.csv", candidate_rows)

    lines = [
        "# LoRA Final Model Selection",
        "",
        "This protocol uses validation macro-F1 for final configuration selection. Test metrics are reported after selection and should not be used to choose the final configuration.",
        "",
        "## Candidate Summary",
        "",
        "| Candidate | Group | Complete | Val Macro-F1 Mean | Val Macro-F1 Std | Test Macro-F1 Mean | Test Macro-F1 Std | Test Accuracy Mean |",
        "|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    complete_candidates = [row for row in candidate_rows if row.get("complete")]
    if complete_candidates:
        ...
        lines.extend(
            [
                ...
                "",
                "Selection rule: highest mean validation macro-F1 across seeds. Test metrics remain confirmatory.",
            ]
        )
    lines.extend(
        [
            ...
            "## Output Files",
            "",
            f"- Per-run CSV: `{OUTPUT_ROOT / 'runs.csv'}`",
            f"- Candidate CSV: `{OUTPUT_ROOT / 'candidates.csv'}`",
        ]
    )
